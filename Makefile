...
MIGR_SQL := migrations/001_init_schema.sql
...
.PHONY: db-reset
db-reset: docker-down ## Перезапустить миграцию и создать базу данных с нуля
  @echo "$(YELLOW)Resetting database...$(NC)"
  $(DOCKER_COMPOSE) down -v # Удалить контейнеры и связанные с ними тома
  $(DOCKER_COMPOSE) up -d postgres # Запустить PostgreSQL
  @echo "$(GREEN)Waiting for PostgreSQL to be ready...$(NC)"
  @sleep 5
  @echo "$(GREEN)PostgreSQL started$(NC)"
  @echo "$(GREEN)Running migrations...$(NC)"
  # docker exec -it blog_postgres psql -U bloguser -d blogdb -f /path/to/migrations.sql
  # docker exec -it blog_postgres psql -U bloguser -d blogdb -f /migrations/001_init_schema.sql

  docker exec -it blog_postgres psql -U bloguser -d blogdb -f MIGR_SQL
